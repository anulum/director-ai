...
# Director-Class AI — run_boolq_training
#!/usr/bin/env python3
"""Fine-tune FactCG-DeBERTa-v3-Large on BoolQ for QA grounding.

BoolQ (Clark et al. 2019): ~16K yes/no question-passage pairs from Google.
Maps to NLI: passage=premise, question=hypothesis, yes=entailment.

Usage:
    python run_boolq_training.py [--resume]
"""
...
OUTPUT_DIR = "/home/director-ai/models/factcg-boolq"
TEMPLATE = (
...
def load_boolq():
    ...
    def convert(example):
        # BoolQ: label 0=False, 1=True → True=supported
        label = example["label"]
        text = TEMPLATE.format(text_a=example["passage"], text_b=example["question"])
    ...
    print(f"BoolQ loaded: train={len(train)}, val={len(val)}, test={len(test)}")
...
def main():
    ...
    print("=== BoolQ Fine-Tuning ===")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)
    model = AutoModelForSequenceClassification.from_pretrained(BASE_MODEL, num_labels=2)
    train_ds, val_ds, test_ds = load_boolq()
