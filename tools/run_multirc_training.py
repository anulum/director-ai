...
# Director-Class AI — run_multirc_training
#!/usr/bin/env python3
"""Fine-tune FactCG-DeBERTa-v3-Large on MultiRC for multi-sentence reading comprehension.

MultiRC (Khashabi et al. 2018): ~27K question-answer pairs over multi-sentence
paragraphs. Tests multi-hop reasoning — relevant to document-level fact checking.

Usage:
    python run_multirc_training.py [--resume]
"""
...
OUTPUT_DIR = "/home/director-ai/models/factcg-multirc"
TEMPLATE = (
...
def load_multirc():
    ...
    def convert(example):
        # MultiRC: paragraph + question + answer candidate, label 0/1
        paragraph = example["paragraph"]
        question = example["question"]
        answer = example["answer"]
        premise = f"{paragraph}\n\nQuestion: {question}"
        text = TEMPLATE.format(text_a=premise, text_b=answer)
    ...
    print(f"MultiRC loaded: train={len(train)}, val={len(val)}, test={len(test)}")
...
def main():
    ...
    print("=== MultiRC Fine-Tuning ===")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)
    model = AutoModelForSequenceClassification.from_pretrained(BASE_MODEL, num_labels=2)
    train_ds, val_ds, test_ds = load_multirc()
