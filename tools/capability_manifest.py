...
def _default_labels() -> dict[str, str]:
    ...
    return {
        ...
        "python_model_source_modules": "Python capability source modules",
        "python_model_classes": "Python capability classes",
        "model_documentation_pages": "Capability documentation pages",
        "rust_pyo3_model_wrappers": "Rust PyO3 bindings",
...
class CapabilityManifestConfig:
    ...
    exclude_source_parts: tuple[str, ...]
    labels: dict[str, str]
...
def load_config(
    ...
    """Load portable manifest config with Director-AI-compatible defaults."""
    ...
    return CapabilityManifestConfig(
        project_label=str(raw.get("project_label", "Director-AI")),
        schema_version=str(
        ...
        package_root=Path(paths.get("package_root", "src/director_ai")),
        model_sources=Path(paths.get("model_sources", "src/director_ai")),
        model_docs=Path(paths.get("model_docs", "docs-site/api")),
        ...
        rust_wrappers=Path(
            paths.get("rust_wrappers", "backfire-kernel/crates/backfire-ffi/src")
        ),
        exclude_doc_parts=tuple(
            str(part)
            for part in raw.get("exclude_doc_parts", ["internal", "_generated"])
        ),
        exclude_source_parts=tuple(
            str(part)
            for part in raw.get(
                "exclude_source_parts",
                [
                    "__pycache__",
                    "director_pb2.py",
                    "director_pb2_grpc.py",
                    "director_pb2.pyi",
                ],
            )
        ),
...
def build_capability_manifest(
    ...
    python_model_sources = _python_model_sources(
        paths.models_root,
        repo=repo,
        exclude_parts=config.exclude_source_parts,
    )
    python_model_classes = _python_model_classes(
        paths.models_root,
        repo=repo,
        exclude_parts=config.exclude_source_parts,
    )
    rust_pyo3_wrappers = (
        _rust_pyo3_wrapper_names(paths.pyo3_wrappers)
        if paths.pyo3_wrappers.exists()
        else []
    )
    ...
    docs_pages = _markdown_docs(
        paths.docs_root, repo=repo, exclude_parts=config.exclude_doc_parts
    )
...
def validate_manifest(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if isinstance(models, dict) and isinstance(counts, dict):
        ...
        _check_count(
            errors, counts, "python_model_classes", models.get("python_classes")
        )
        _check_count(
            errors,
            counts,
            "model_documentation_pages",
            models.get("documentation_pages"),
        )
        _check_count(
            errors, counts, "rust_pyo3_model_wrappers", models.get("rust_pyo3_wrappers")
        )
...
def _public_exports(init_path: Path) -> list[str]:
    ...
    dict_assignments: dict[str, list[str]] = {}
    for node in tree.body:
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name) and isinstance(node.value, ast.Dict):
                    dict_assignments[target.id] = _literal_string_keys(node.value)
        elif (
            isinstance(node, ast.AnnAssign)
            and isinstance(node.target, ast.Name)
            and isinstance(node.value, ast.Dict)
        ):
            dict_assignments[node.target.id] = _literal_string_keys(node.value)
    for node in tree.body:
        if isinstance(node, ast.Assign):
            for target in node.targets:
                if isinstance(target, ast.Name) and target.id == "__all__":
                    literal_exports = _literal_string_list(node.value)
                    if literal_exports:
                        return sorted(literal_exports)
                    derived_exports = _sorted_name_keys(node.value, dict_assignments)
                    if derived_exports:
                        return sorted(derived_exports)
    return []
...
def _literal_string_keys(node: ast.Dict) -> list[str]:
    values: list[str] = []
...
def _sorted_name_keys(
    node: ast.AST, dict_assignments: dict[str, list[str]]
) -> list[str]:
    ...
    if isinstance(arg, ast.Name):
        return dict_assignments.get(arg.id, [])
    return []


def _python_model_sources(
    models_root: Path,
    *,
    repo: Path,
...
def _excluded_source_path(
    path: Path, root: Path, exclude_parts: tuple[str, ...]
) -> bool:
    relative = path.relative_to(root)
    parts = set(relative.parts)
    return bool(parts.intersection(exclude_parts) or path.name in exclude_parts)


def _rust_pyo3_wrapper_names(path: Path) -> list[str]:
    paths = sorted(path.rglob("*.rs")) if path.is_dir() else [path]
    names: set[str] = set()
    for rust_path in paths:
        text = rust_path.read_text(encoding="utf-8")
        names.update(re.findall(r'py_neuron_default!\("([^"]+)"', text))
        names.update(
            re.findall(r'#\[pyclass\(\s*name\s*=\s*"([^"]+)"', text, flags=re.S)
        )
        names.update(
            re.findall(r"#\[pyfunction(?:\([^]]*\))?\]\s*(?:pub\s+)?fn\s+(\w+)", text)
        )
        names.update(re.findall(r"wrap_pyfunction!\((\w+),", text))
        names.update(re.findall(r"m\.add_class::<(\w+)>\(\)", text))
    return sorted(names)
