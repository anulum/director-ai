#!/usr/bin/env python3
"""Fine-tune FactCG-DeBERTa-v3-Large on CommitmentBank for textual entailment.

CB (de Marneffe et al. 2019): 250 naturally-occurring discourses annotated for
entailment, contradiction, neutral. High-quality 3-class NLI from SuperGLUE.

Usage:
    python run_cb_training.py [--resume]
"""
...
OUTPUT_DIR = "/home/director-ai/models/factcg-cb"
TEMPLATE = (
...
def load_cb():
    ...
    def convert(example):
        # CB: 0=entailment, 1=contradiction, 2=neutral → binary
    ...
    print(f"CB loaded: train={len(train)}, val={len(val)}, test={len(test)}")
...
def main():
    ...
    print("=== CommitmentBank Fine-Tuning ===")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)
    model = AutoModelForSequenceClassification.from_pretrained(BASE_MODEL, num_labels=2)
    train_ds, val_ds, test_ds = load_cb()
    ...
    training_args = TrainingArguments(
        output_dir=OUTPUT_DIR, num_train_epochs=20, per_device_train_batch_size=8,
        per_device_eval_batch_size=32, learning_rate=1e-5, weight_decay=0.01,
        warmup_ratio=0.2, eval_strategy="epoch", save_strategy="epoch",
    ...
    result = {
        "dataset": "cb_superglue", "base_model": BASE_MODEL,
        "test_balanced_accuracy": test_result["eval_balanced_accuracy"],
        "test_accuracy": test_result["eval_accuracy"],
        "training_time_minutes": round(elapsed / 60, 1), "epochs": 20, "status": "COMPLETE",
    }
