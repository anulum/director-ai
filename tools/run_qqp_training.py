...
# Director-Class AI — run_qqp_training
#!/usr/bin/env python3
"""Fine-tune FactCG-DeBERTa-v3-Large on QQP for paraphrase detection.

QQP (Quora Question Pairs): ~400K question pairs labeled as duplicate/not.
Maps to NLI: duplicate=entailment (semantically equivalent), not=contradiction.

Usage:
    python run_qqp_training.py [--resume]
"""
...
OUTPUT_DIR = "/home/director-ai/models/factcg-qqp"
TEMPLATE = (
...
def load_qqp():
    ...
    def convert(example):
        # QQP: label 1=duplicate (paraphrase), 0=not duplicate
        label = example["label"]
        text = TEMPLATE.format(text_a=example["question1"], text_b=example["question2"])
...
def main():
    ...
    print("=== QQP Fine-Tuning ===")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)
    model = AutoModelForSequenceClassification.from_pretrained(BASE_MODEL, num_labels=2)
    train_ds, val_ds, test_ds = load_qqp()
