#!/usr/bin/env python3
"""Fine-tune FactCG-DeBERTa-v3-Large on MRPC for paraphrase detection.

MRPC (Dolan & Brockett 2005): ~5.8K sentence pairs from news articles,
labeled as semantic paraphrases or not.

Usage:
    python run_mrpc_training.py [--resume]
"""
...
OUTPUT_DIR = "/home/director-ai/models/factcg-mrpc"
TEMPLATE = (
...
def load_mrpc():
    ...
    def convert(example):
        # MRPC: 1=paraphrase (equivalent), 0=not paraphrase
        label = example["label"]
    ...
    print(f"MRPC loaded: train={len(train)}, val={len(val)}, test={len(test)}")
...
def main():
    ...
    print("=== MRPC Fine-Tuning ===")
    tokenizer = AutoTokenizer.from_pretrained(BASE_MODEL)
    model = AutoModelForSequenceClassification.from_pretrained(BASE_MODEL, num_labels=2)
    train_ds, val_ds, test_ds = load_mrpc()
    ...
    result = {
        "dataset": "mrpc_glue", "base_model": BASE_MODEL,
