...
# Director-Class AI — generic JarvisLabs GPU benchmark runner
"""Provision a JarvisLabs GPU, run one benchmark, and tear down.

Provisions a small GPU via the current jlclient API, has the remote
git-clone the tag and run the selected repo-relative benchmark in a fresh
Python 3.11 conda environment, downloads its JSON artefact, and destroys the
instance in a ``finally`` block so a failure never leaks a paid GPU. The remote
recipe (region, GPU availability, py3.11, ensurepip) is encoded in
:data:`REMOTE_SCRIPT`.

Usage::

    export JARVISLABS_TOKEN=...
    python tools/run_remote_benchmark_gpu.py --gpu A30 --tag v3.18.1 \
        --script benchmarks/ragtruth_eval.py --remote-deps datasets
"""
...
logger = logging.getLogger("DirectorAI.RemoteBenchmarkGPU")
...
set -euo pipefail
echo "=== Director-AI remote benchmark (JarvisLabs) ==="
...
$CR python __SCRIPT__ --out /home/director-ai/benchmark_result.json --git-sha "$GIT_SHA"
echo "=== done; artefact at /home/director-ai/benchmark_result.json ==="
"""
...
def provision(gpu_type: str, storage: int, token: str) -> Any:
    ...
    inst = Instance.create(
        ...
        num_gpus=1,
        storage=storage,
        name="director-ai-remote-benchmark",
        is_reserved=True,
        duration="hour",
        region=NOIDA_REGION,
    )
...
def run_benchmark(
    ...
    """Write and execute the benchmark script on the remote, streaming output.
    ...
    *script_path* is the repo-relative benchmark run on the GPU; it must accept
    ...
    remote = "/tmp/run_director_benchmark.sh"
    subprocess.run(
    ...
    logger.info("Running benchmark on the GPU (streaming)...")
...
def download_artefact(ssh_str: str, out: str) -> None:
    ...
    subprocess.run(
        ...
        f"{host}:/home/director-ai/benchmark_result.json {dest}",
        shell=True,
        check=True,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        run_benchmark(inst.ssh_str, args.tag, args.script, tuple(args.remote_deps))
