...
For first deployments, use the Python-only quickstart generated by
`director-ai quickstart --run`. For authenticated production scaffolding with
tenant routing, audit stores, signed KB writes, rate limiting, authenticated
metrics, and the Prometheus profile, run:

```bash
director-ai quickstart --profile production
```

GPU, TensorRT, gateway, formal-verifier, and WASM paths are advanced runtimes;
