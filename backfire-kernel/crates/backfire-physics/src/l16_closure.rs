...
pub struct L16StepResult {
    ...
    pub refusal: bool,
    pub h_rec: f64,
    /// Numerical dH_rec/dt via finite difference over history window.
    pub dh_rec_dt: f64,
    /// True when dH_rec/dt ≤ 0 (Lyapunov condition holds).
...
impl L16Controller {
    ...
    pub fn compute_h_rec(
        ...
        let (sum_sin, sum_cos, sum_sin2, sum_cos2) =
            theta
                .iter()
                .fold((0.0, 0.0, 0.0, 0.0), |(ss, sc, ss2, sc2), &th| {
                    let s = th.sin();
                    let c = th.cos();
    ...
    /// Estimate dH_rec/dt via least-squares linear regression on the
    /// most recent h_rec_window values. Returns slope (negative = stable).
    pub fn compute_dh_dt(&self, dt: f64) -> f64 {
        let hist = &self.state.h_rec_history;
        let w = self.h_rec_window;
        if hist.len() < 2 || w < 2 {
            return 0.0;
        }
        let n = hist.len().min(w);
        ...
        // Slope is in H_rec units per index step; convert to per-second
    ...
    pub fn step(
        ...
        let dh_rec_dt = self.compute_dh_dt(dt);
        let lyapunov_stable = dh_rec_dt <= 0.0;
...
mod tests {
    ...
    fn test_refusal_activates_on_rising_h_rec() {
        ...
        assert!(
            ctrl.check_refusal(),
            "Refusal should activate on rising H_rec"
        );
    ...
    fn test_refusal_deactivates_on_flat_h_rec() {
        ...
        ctrl.check_refusal();
        assert!(
            !ctrl.state.refusal_active,
            "Refusal should deactivate on flat H_rec"
        );
    ...
    fn test_plv_window_truncation() {
        ...
        assert_eq!(
            ctrl.state.plv_buffer.len(),
            5,
            "PLV buffer should be truncated to window"
        );
    }

    #[test]
    fn test_dh_dt_rising_positive() {
        let mut ctrl = L16Controller::default_params();
        // Push monotonically rising H_rec values
        for i in 0..10 {
            ctrl.state.h_rec_history.push(i as f64 * 0.1);
        }
        let dh = ctrl.compute_dh_dt(0.01);
        assert!(
            dh > 0.0,
            "Rising H_rec should give positive dH/dt, got {dh}"
        );
    }

    #[test]
    fn test_dh_dt_falling_negative() {
        let mut ctrl = L16Controller::default_params();
        for i in 0..10 {
            ctrl.state.h_rec_history.push(1.0 - i as f64 * 0.1);
        }
        let dh = ctrl.compute_dh_dt(0.01);
        assert!(
            dh < 0.0,
            "Falling H_rec should give negative dH/dt, got {dh}"
        );
    }

    #[test]
    fn test_dh_dt_flat_zero() {
        let mut ctrl = L16Controller::default_params();
        for _ in 0..10 {
            ctrl.state.h_rec_history.push(0.5);
        }
        let dh = ctrl.compute_dh_dt(0.01);
        assert!(dh.abs() < 1e-9, "Flat H_rec should give dH/dt≈0, got {dh}");
    }

    #[test]
    fn test_lyapunov_stable_in_step_result() {
        let mut ctrl = L16Controller::default_params();
        let theta = vec![0.5; 16];
        let costs = L16CostInputs {
            c7_symbolic: 0.1,
        ...
        // First step: no history → dH/dt = 0 → stable
        let r1 = ctrl.step(&theta, &[], 0, &[], 0, 0.7, 0.65, &costs, 0.01);
        assert!(
            r1.lyapunov_stable,
            "First step should be Lyapunov stable (no history)"
        );
    }
}
