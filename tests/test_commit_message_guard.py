...
def test_commit_message_requires_arcane_sapience_authorship_line(tmp_path: Path) -> None:
    ...
    assert "exactly one required authorship line" in result.stderr
...
def test_commit_message_rejects_legacy_coauthored_trailers(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    ...
    assert result.returncode == 1
    assert "must not use legacy Co-Authored-By trailers" in result.stderr
...
def test_commit_message_rejects_blocked_agent_identity(tmp_path: Path) -> None:
    ...
    message_path = _message(
        repo,
        f"Generated by {blocked_identity}\n\n{REQUIRED_AUTHORSHIP_LINE}\n",
    )
...
def test_commit_message_allows_provider_names_as_subject_matter(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
    message_path = _message(
        repo,
        f"Repair OpenAI and Anthropic SDK clients\n\n{REQUIRED_AUTHORSHIP_LINE}\n",
    )
...
def test_commit_message_accepts_single_required_authorship_line(tmp_path: Path) -> None:
    repo = _repo(tmp_path)
