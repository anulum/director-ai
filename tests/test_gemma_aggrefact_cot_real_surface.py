...
"""Real subprocess coverage for the Gemma AggreFact CoT evaluator."""
...
SCRIPT_PATH = PROJECT_ROOT / "benchmarks" / "gemma_aggrefact_cot.py"
...
class Llama:
    ...
    def create_chat_completion(self, *, messages, max_tokens, temperature):
        ...
        return {{"choices": [{{"message": {{"content": "Reasoning sentence.\\nANSWER: " + verdict}}}}]}}
""",
        encoding="utf-8",
    )
    return call_log
...
def _run_cot_cli(
    ...
    *,
    ...
    """Run the production CoT evaluator script with local protocol modules."""
    protocol_dir = tmp_path / "protocol"
    call_log = _write_protocol_modules(protocol_dir)
    report_path = tmp_path / "results" / "gemma_cot.json"
    env = os.environ.copy()
...
def test_gemma_aggrefact_cot_unit_guard_has_real_cli_companion() -> None:
    """Ensure the CoT unit guard is backed by this subprocess companion."""
    assert KNOWN_TEST_SURFACE_CLASSIFICATIONS["tests/test_gemma_aggrefact_cot.py"] == (
        "unit-guard-with-companion",
        "ML/export/eval Gemma AggreFact CoT evaluator guard with companion "
        "tests/test_gemma_aggrefact_cot_real_surface.py",
    )


def test_gemma_aggrefact_cot_cli_writes_report_from_protocol_modules(
    tmp_path: Path,
) -> None:
    """Exercise the CoT evaluator through its benchmark CLI contract."""
    completed, report_path, call_log = _run_cot_cli(tmp_path)

    assert completed.returncode == 0, completed.stderr
    report = _read_json(report_path)
    assert report["model"] == "/models/local-cot.gguf"
    assert report["prompt_style"] == "chain-of-thought (1-2 sentences + ANSWER)"
    ...
    assert report["sample_responses"] == [
        "Reasoning sentence.\nANSWER: SUPPORTED",
    ...
    assert calls[0]["init"]["model_path"] == "/models/local-cot.gguf"
    assert calls[0]["init"]["n_ctx"] == 2048
    assert calls[0]["init"]["n_threads"] == 3
    assert {call["max_tokens"] for call in calls[1:]} == {32}
    ...
    assert "ANSWER: NOT_SUPPORTED" in prompt_text


def test_gemma_aggrefact_cot_cli_rejects_empty_dataset(tmp_path: Path) -> None:
    """Reject empty CoT inputs before reporting or dividing by zero."""
    completed, report_path, _call_log = _run_cot_cli(tmp_path, empty_dataset=True)
