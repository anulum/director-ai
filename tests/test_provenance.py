...
class TestCitationFact:
    def test_hash_autoderived(self) -> None:
    ...
    def test_matching_persisted_hash_is_accepted(self) -> None:
        """Persisted facts with a matching content hash should reload cleanly."""
        original = CitationFact(
            source_id="wiki",
            content="the sky is blue",
            timestamp=0.0,
        )

        restored = CitationFact(
    ...
    def test_source_id_collision_rejected(self) -> None:
...
class TestMerkleTree:
    ...
    def test_proof_verification_all_leaves(self) -> None:
    ...
    def test_unknown_fact(self) -> None:
    ...
    def test_proof_is_frozen_dataclass(self) -> None:
        tree = MerkleTree(_facts(4))
        proof = tree.proof_for(tree.facts()[0])
...
class TestProvenanceChain:
    ...
    def test_tampered_tag_detected(self) -> None:
        chain = ProvenanceChain(secret=b"s" * 32)
        chain.append(merkle_root="root-1")
    ...
    def test_tampered_parent_hash_detected(self) -> None:
        chain = ProvenanceChain(secret=b"s" * 32)
        chain.append(merkle_root="root-1")
        chain.append(merkle_root="root-2")
    ...
    def test_hmac_chain_error_class(self) -> None:
        assert issubclass(HmacChainError, ValueError)

    def test_concurrent_appends_are_atomic(self) -> None:
        chain = ProvenanceChain(secret=b"s" * 32)
    ...
    def test_snapshot_returns_append_only_audit_entries(self) -> None:
        chain = ProvenanceChain(secret=b"s" * 32)
...
class TestSourceCredibility:
    def test_prior_for_unseen_source(self) -> None:
        cred = SourceCredibility(prior=0.7, clock=_FakeClock())
        assert cred.score("new-source") == 0.7

    def test_observation_pulls_toward_signal(self) -> None:
        clock = _FakeClock()
    ...
    def test_half_life_returns_halfway_point(self) -> None:
        clock = _FakeClock(start=0.0)
    ...
    def test_multiple_observations_accumulate_count(self) -> None:
        clock = _FakeClock()
    ...
    def test_source_score_validation(self) -> None:
    ...
    def test_observe_validation(self) -> None:
    ...
    def test_score_holds_current_value_when_clock_moves_backwards(self) -> None:
...
class TestProvenanceVerifier:
    ...
    def test_healthy_response_all_ok(self) -> None:
    ...
    def test_integrity_failure_surfaced(self) -> None:
        verifier = self._verifier()
    ...
    def test_low_source_score_flagged(self) -> None:
    ...
    def test_chain_entry_included(self) -> None:
        verifier = self._verifier()
    ...
    def test_bad_min_source_score(self) -> None:
