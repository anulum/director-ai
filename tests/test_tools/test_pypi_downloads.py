...
# Director-AI — bounded PyPI metrics tests
"""Tests for Director-AI's fail-closed PyPI download history."""
...
_SPEC = importlib.util.spec_from_file_location(
    "director_ai_pypi_downloads", _MODULE_PATH
)
...
def test_detect_package_reads_project_contract(tmp_path: Path) -> None:
    ...
    assert downloads.detect_package(pyproject) == "director-ai"
...
def test_project_writer_rejects_every_alternate_target_before_fetch(
    ...
    downloads.require_project_csv_target(
        "director-ai", Path("downloads/director-ai.csv")
    )
    for package, csv_path in (
        ("other", Path("downloads/director-ai.csv")),
        ("director-ai", Path("downloads/other.csv")),
        ("director-ai", Path("downloads/../downloads/director-ai.csv")),
        ("director-ai", tmp_path / "downloads/director-ai.csv"),
    ):
...
def test_http_get_uses_fixed_host_bounded_read_and_closes(
    ...
    assert downloads._http_get("director-ai") == b'{"ok": true}'
    assert connection.request_args == (
        "GET",
        "/api/packages/director-ai/overall",
    )
    assert connection.request_kwargs == {
        "headers": {
            "Accept": "application/json",
            "User-Agent": "director-ai-metrics/1",
        }
    }
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            downloads.RetryableSnapshotError,
            "transient HTTP 429",
        ),
...
def test_http_get_fails_closed_and_closes(
    ...
    with pytest.raises(error_type, match=message):
        downloads._http_get("director-ai")
    assert connection.closed
...
def test_fetch_overall_rejects_invalid_json_contract(payload: bytes) -> None:
    ...
    with pytest.raises(downloads.DownloadSnapshotError):
        downloads.fetch_overall("director-ai", lambda package: payload)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            b'"downloads":19}],"package":"other","package":"director-ai",'
            b'"type":"overall_downloads"}'
        ),
        (
            b'{"data":[{"category":"with_mirrors","date":"2026-07-17",'
            b'"downloads":1,"downloads":19}],"package":"director-ai",'
            b'"type":"overall_downloads"}'
        ),
    ],
)
def test_duplicate_remote_object_names_fail_before_csv_write(
    ...
    assert (
        downloads.main(
            ["--package", "director-ai", "--csv", str(csv_path)],
...
def test_remote_schema_fails_closed_on_identity_or_partial_data(
    ...
    with pytest.raises(downloads.DownloadSnapshotError, match=message):
        downloads.fetch_overall("director-ai", lambda package: _payload_bytes(payload))
...
def test_fetch_overall_returns_complete_validated_rows() -> None:
    ...
    assert downloads.fetch_overall(
        "director-ai", lambda package: _payload_bytes(_SAMPLE)
    ) == {
...
def test_remote_schema_preserves_legitimate_sparse_without_mirrors() -> None:
    ...
    payload["data"] = [
        ...
        if not (row["date"] == "2026-07-17" and row["category"] == "without_mirrors")
    ]
    assert downloads.fetch_overall(
        "director-ai", lambda package: _payload_bytes(payload)
    )["2026-07-17"] == {"with_mirrors": 19}
...
def test_missing_csv_and_empty_summary_are_explicit(tmp_path: Path) -> None:
    ...
    assert downloads.summary("director-ai", {}) == (
        "director-ai: no download data available yet"
    )
    assert "without_mirrors=n/a" in downloads.summary(
        "director-ai", {"2026-07-17": {"with_mirrors": 19}}
    )
...
def test_merge_and_main_preserve_history(
...
def test_main_failure_keeps_existing_series(
    ...
    assert (
        downloads.main(["--package", "director-ai", "--csv", str(csv_path)], fail_fetch)
        == 1
    )
...
def test_main_print_package_and_requires_csv(
    ...
    assert capsys.readouterr().out.strip() == "director-ai"
    with pytest.raises(SystemExit) as exc_info:
        downloads.main(["--package", "director-ai"])
...
def test_transient_failures_retry_then_soft_skip(
    ...
    result = downloads.fetch_overall_with_retry("director-ai", recover, waits.append)
...
def test_workflow_has_one_push_only_bounded_writer_and_pinned_actions() -> None:
    ...
    workflow = (_ROOT / ".github" / "workflows" / "pypi-downloads.yml").read_text(
        encoding="utf-8"
    )
    ...
    assert 'readonly package="director-ai"' in workflow
    assert 'readonly csv_path="downloads/director-ai.csv"' in workflow
    ...
    assert workflow.index("GH_TOKEN:") > workflow.index(
        "Push only the reviewed metrics ref"
    )
    assert (
        "actions/upload-artifact@043fb46d1a93c77aae656e7c1c64a875d1fc6a0a" in workflow
    )
    assert "director-ai-metrics/downloads/director-ai.csv" in workflow
