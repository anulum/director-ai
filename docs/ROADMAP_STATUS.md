...
<!-- Director-AI - Roadmap Status Reconciliation -->
...
| R2 | Lite Scorer v2 training completion and real artefact evaluation | Partly implemented: durable training launcher, run manifest, held-out builder, guarded evaluator, ONNX export runner, evidence recorder, validator, recorded evidence packet, model-card hash, benchmark-claim review, and passing recorded-evidence gate are present. No public scored release claim is approved. | Operator approval for any scored release claim, with exact permitted wording and release surfaces |
| R3 | FrontierFail broader sourced production-failure corpus | Partly implemented: FrontierFail seed packet now includes 10 synthetic regressions and 6 public incident intake rows with dated evidence, category/domain/publisher/evidence-reference diversity gates, public metadata requirements, and no public benchmark eligibility claim | External corpus-quality review, preserved validation outputs, and a claim-guarded benchmark card |
| R4 | PINT official upstream run and evidence | Runner-ready: seed packet, seed smoke, validator, and local official-export evidence runner are present; no official upstream export has been executed and no public score claim is approved | Official upstream dataset export, execution output from `tools/run_pint_official_export.py`, preserved dataset/result hashes, and claim-guarded benchmark card |
| R5 | Hugging Face Space live deployment push | Deployment-smoke gate ready; live push remains manual/operator-approved. `demo/hf_space_deployment_smoke.toml` records no public live-demo claim until `deployment_status=published`, `smoke_status=passed`, and dated smoke evidence exists. | Published Space URL plus deployment smoke evidence |
| R6 | Polar storefront environment-specific live deployment smoke | Deployment-smoke gate ready; live work remains environment-specific. `security/polar_deployment_smoke_packet.toml` keeps checkout/commercial claims false until USD checkout, portal, webhook, and licence-validation smoke checks pass with dated evidence and no committed secrets. | Live USD checkout, portal, webhook, and licence validation smoke records without committed secrets |
