...
| R16 | Formal + symbolic depth expansion | Partly implemented: formal/code adapter production paths now have local evidence for DPLL formula halts/allows, Lean runner contract, Z3 optional-profile gate or actual run when `[formal]` is installed, code-contract ordering, tenant-safe serialisation, and Customer Model Factory release-gate formal symbolic evidence contract is present | External Lean proof run, actual Z3 packet under `[formal]` in release evidence, operator-owned math/code/numeric domain contracts, code-contract packet, and operator sign-off |
